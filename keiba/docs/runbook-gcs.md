...
### 過去5年分のダービーを一括取得（バックテスト用）

```bash
# 2021〜2025 のダービーを過去レースモードで一括取得（答え合わせ用 actual.csv も保存）
python3 -m keiba.cli fetch --derby-years 2021-2025 --past \
    --out gs://keiba_shohei/keiba --wait 1.5
```

`--derby-years` は「東京・2回・12日目・11R」を仮定して race_id を年で展開する:
...
> ⚠️ **開催日目(12日目)は年によってずれることがある**。ずれている年は「出走0頭」と
> 警告が出る。その場合は正しい race_id を netkeiba のレース URL で確認し、
> `--race-ids 202X05021211,...` で個別に指定する。

各年の分析（レース前時点の指標 vs 実着順の答え合わせ）:

```bash
for y in 2021 2022 2023 2024 2025; do
  python3 -m keiba.cli analyze --race-id ${y}05021211 --store gs://keiba_shohei/keiba
done
```

...
def visualize(race_id: str):
    ...
    # 過去ダービーを「全年まとめて1枚」で傾向プロット。指標をラジオで切替え。
    ...
    if past_ids:
        ...
        past_block = (
            f'<h2>📚 過去ダービー（{len(past_ids)}年）の好走傾向</h2>'
            f'<p class="note">過去全年をまとめて、選んだ指標で出走馬をプロットします。'
            f'橙＝実際に3着以内に来た馬。橙が指標のどのあたりに集まるか（破線＝複勝圏の中央値）で、'
            f'複数年に共通する傾向が分かります。</p>'
            f'<p>指標: {radios}</p>'
            f'<div class="card" style="max-width:760px">'
            f'<img id="trendimg" loading="lazy" '

...
## 特長
- **月表示カレンダー**：土日のマスにだけ「昼」「夜」の枠が出ます。土=青/日=赤で
  色分けし、今日は強調、成立枠は緑＋✅で表示します。
- **出欠はプルダウンで選ぶだけ**：その月の土日が一覧で並び、各枠（昼/夜）の
  プルダウンから「⭕参加 / 🔺未定 / ❌不参加 / 未回答」を選びます。
- **「詳細」ボタンで参加者を確認**：押すとその枠の参加者が ○/△/× 別に表示され、
  メール登録済みの参加者には mailto リンクが付きます。

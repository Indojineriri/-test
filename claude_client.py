...
def chat_reply(
    ...
    context = (
        ...
        f"# アジェンダ・メッセージの初期案\n{material_md}\n\n"
        "【重要なルール】\n"
        "・これ以降の会話では、直近のあなたの回答が常に『最新版』です。"
        "ユーザーの修正依頼は必ず最新版に対して適用し、初期案には戻さないでください。\n"
        "・一度削除した項目は、明示的に依頼されない限り復活させないでください。\n"
        "・修正のたびに、変更を反映した『アジェンダ・メッセージの全文（完成形）』を"
        "返してください（差分や該当箇所だけでなく、常に全体像を示す）。\n"
        "・日本語で簡潔に応答してください。"
    )

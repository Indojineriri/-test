...
5. 各ベンダーごとに次の項目を埋める：
   - company / product / summary（**最大 80 文字、1 文**で言い切る）
   - features（製品の特長、最大 3 個 / **各 50 文字以内**）
   - problems_solved（解決する課題、最大 3 個 / **各 40 文字以内**）
   - use_cases（活用例、最大 3 個 / **各 60 文字以内、『見出し: 説明』形式**）
   - url / image_url
   - references（参考リンクのリスト、最大 3 件）。
     リサーチで実際に有益だった一次情報源 URL を選ぶ。
     優先度: ① 製品スペック / データシートページ ② プレスリリース or 業界記事
     ③ デモ動画 or ホワイトペーパー
...
- **文字数制約を厳守**: summary ≤ 80 / feature ≤ 50 / problem ≤ 40 / use_case ≤ 60。
  オーバーすると PPT で枠からはみ出して読めなくなる。
...
- summary は競合と比べて何がユニークかが読み取れる 1 文に圧縮する。
- references は最低 1 件、最大 3 件入れる。実際に fetch_url で読んだ URL から選ぶこと。
